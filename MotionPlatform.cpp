...
void MotionPlatform::update()
{
    ...
    if (NT_GetPosition_S(ntHandle_, channelIndex_, &position_) != NT_OK ||
        NT_GetStatus_S(ntHandle_, channelIndex_, &motionSta_) != NT_OK) {
        qDebug() << "Failed to get position or status, disconnecting...";
        disConnect();
        return;
    }
